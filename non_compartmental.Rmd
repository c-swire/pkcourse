...
### Using \texttt{R} package \texttt{PKNCA} to undertake non-compartmental analysis  

If that all seems like too much hard work and \texttt{R} should just jolly well get on and do for us then you are right! There are a few non-compartmental analysis packages available, we will use \texttt{PKNCA}. Remember you will need to run \texttt{install.packages("PKNCA")} the first time you wish to use this package, once it has been installed you need to run \texttt{library(PKNCA)}. 

To run non-compartmental analysis using this package, we need to give \texttt{PKNCA} some dosing data. We know that all the participants were given a dose of 1000 mg of amoxicillin at time $t=0$. To add a new column (variable) using \texttt{\%>\%} we use the \texttt{mutate} function.  
...
Now we can call and run \texttt{PKNCA}. First we create a sampling (or concentration) object and then a dosing object. In each function we instruct \texttt{PKNCA} of the names of our data object and the relevant variables in it. Then we combine the two and run our non-compartmental analysis. 
