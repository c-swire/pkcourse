...
* Calculate $C_{max}$, $T_{max}$ and $AUC_{0-t}$ for the 'Healthy' dataset

* Calculate $C_{max}$, $T_{max}$ and $AUC_{0-t}$ for the 'Sepsis' dataset

* Estimate $Vd$ and $Cl$ for the 'Healthy' dataset

* Estimate $Vd$ and $Cl$ for the 'Sepsis' dataset
...
The first six lines of the IV dataset are shown here.
...
**How to:** Plot the data for the IV data.
...
Look at the value for $AUC$ produced here and compare it to the one earlier. There is a discrepancy because by default, PKNCA uses logarithmic trapezoid method to calculate $AUC$. 

# Estimating clearance

Clearance can be calculated as $$CL = \frac{Dose}{AUC}$$

\newpage

# Limitations of non-compartmental analysis

```{r traperror, echo=F, fig.cap="Participant A amoxicillin concentration-time profile showing impact of reduced sampling on AUC calculation", fig.width=4.5,fig.height=2.5,fig.align = 'center'}
